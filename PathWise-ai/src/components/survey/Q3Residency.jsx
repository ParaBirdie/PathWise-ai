...
export default function Q3Residency() {
  ...
  const select = (location) => {
    // Store the full state/country name; per-school in-state resolution happens
    // in compareOffers via resolveIsInState(school, residencyState).
    setResidency(location)
    setQuery(location)
  ...
  return (
    <QuestionCard
      ...
      subtitle="We use this to determine in-state tuition eligibility for each school you selected."
      onNext={goNext}
